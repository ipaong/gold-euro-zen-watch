...
-- Drop existing primary key constraint on device_id
ALTER TABLE public.app_settings
  DROP CONSTRAINT IF EXISTS app_settings_pkey;

-- Add surrogate id primary key
ALTER TABLE public.app_settings
...
-- Add UNIQUE constraint on user_id for onConflict resolution
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Maintain full access for service_role
...
-- 8. Add strict per-operation RLS policies using (select auth.uid()) = user_id
...
-- Prediction Results: INSERT verifies ownership of referenced prediction
